...
int coin_count = 0;
int score_count = 0;
int cat_lives = 3; // متغیر گلوبال برای جان گربه
int offsetX = 0;
int mapindex;
...
void printBoard(char board[][64], int corY)
{
    ...
    printf("coin: %d\nscore: %d\nlives: %d\n", coin_count, score_count, cat_lives);
}
...
void fall(int *i, int *j, char board[][64], HANDLE *hStdout, int corY, int block5Removed, int *block5_i, int *block5_j, DWORD *lastBlock5Update,int block5Delay)
{
    while (1)
    {
        if (board[*i + 1][*j] == ' ' || board[*i + 1][*j] == '3' || board[*i + 1][*j] == '5')
        {
            if (!block5Removed)
            {
                moveBlock5(board, block5_i, block5_j, lastBlock5Update, block5Delay, i, j);
            }
            COORD origin = {0, 1};
...
void jump(int *i, int *j, char board[][64], HANDLE *hStdout, int corY, int block5Removed, int *block5_i, int *block5_j, DWORD *lastBlock5Update,int block5Delay)
{
    int jumpHeight = 4;
    for (int f = 1; f <= jumpHeight; f++)
    {
        ...
        if (!block5Removed)
        {
            moveBlock5(board, block5_i, block5_j, lastBlock5Update, block5Delay, i, j); // حرکت رندوم بلوک ۵
        }
        if (board[*i - 1][*j] == '!')
        {
...
void map1()
{
    ...
    while (1)
    {
        ...
        printBoard(board, 15);
        //&block5_i, &block5_j, &lastBlock5Update, block5Delay, &catY, &catX
        if (_kbhit())
        {
            char ch = _getch();
            if (ch == 'w' && catY > 0)
                jump(&catY, &catX, board, &hStdout, 15, block5Removed, &block5_i, &block5_j, &lastBlock5Update, block5Delay);
            ...
            fall(&catY, &catX, board, &hStdout, 15, block5Removed, &block5_i, &block5_j, &lastBlock5Update, block5Delay);
        }
