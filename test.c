...
void map2()
{
    ...
    while (!game_over) // Continue until game over
    {
        ...
        if (_kbhit())
        {
            ...
            else if (ch == 's' && catY < 26 && sizemario == 2)
            {
                board[catY2][catX2] = ' ';
                flags = 1;
                Sleep(20);
...
void drawfirstlevel(int select)
{
    // PlaySoundA(TEXT("C:\\Users\\User\\Downloads\\Music\\coins.wav"), NULL, SND_FILENAME | SND_ASYNC);
...
void levelfailed()
{
    ...
    int select = 0;
    char ch;
    // PlaySoundA(TEXT("C:\\Users\\User\\Downloads\\Music\\Game-Over.wav"), NULL, SND_FILENAME | SND_ASYNC);
